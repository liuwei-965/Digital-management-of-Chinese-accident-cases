...
class LR_RenderText():
    ...
    def render(self, db_dic):
        ...
        for i in range(len(db_dic['间接原因'])):
            self.text.insert('end', db_dic['间接原因'][i]+'\n')  #插入案例文本
            #self.text.insert("1.0", '\n')
        #print("文本：%s" %self.text)
        ...
        for x in db_dic:
            if 'occur' in x:
                if db_dic[x]["inChain"]:
                    #print("到这里了")
                    for i in range(len(db_dic[x]['text'])):
                        #print(db_dic[x]["str_begin"][i])
                        tagbegin = "{}.{}".format(db_dic[x]["str_in"]+1, db_dic[x]["str_begin"][i])   #动词开始索引
                        #print(tagbegin )
                        tagend = "{}.{}".format(db_dic[x]["str_in"]+1, db_dic[x]["str_end"][i])    #动词结束索引
                        #print(coordinates)
                        self.text.tag_add("occur", tagbegin, tagend)  # 添加标记
                        '''if i%2 == 0:
                            self.text.tag_add("occur1", tagbegin, tagend)     #添加标记
                        else:
                            self.text.tag_add("occur2", tagbegin, tagend)
                        i = i+1'''
            self.text.tag_configure("occur", underline=True, lmargin1=2, foreground="blue", relief='raised',borderwidth=1)      #标记突出显示格式
    ...
    def focus_one_occur(self, click_index,dbcursor):
        ...
        jieba.load_userdict('./temp/userdict.txt')
